...
Shader "BoxClip/Skybox/Procedural" {
Properties {
...
SubShader {
    Tags { "Queue"="AlphaTest+25" "RenderType"="Opaque" "PreviewType"="Skybox" "DisableBatching"="True" }
    Cull [_CullMode]
    ZWrite On
 ...
 ENDCG
    ...
    Pass {
        ...
        // #define _WorldSpaceLightPos0 normalize(float3(0.2,0.8,-0.1))
        ...
        BOXCLIP_GEOM_SHADER_LOCALPOS(xv2f, i.posLocal.xyz)

        xv2f vert (appdata_t v)
        {
            xv2f OUT;
            UNITY_SETUP_INSTANCE_ID(v);
            UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(OUT);
            boxPreprocessVertex(v.vertex);
            OUT.pos = UnityObjectToClipPos(v.vertex);
            OUT.posLocal = v.vertex;
            return OUT;
        }

        v2f vertpixel(xv2f v) {
            v2f OUT;
            float4 vvertex = v.posLocal;
            OUT.posWorld = mul(unity_ObjectToWorld, vvertex);
            vvertex = float4(normalize(OUT.posWorld.xyz - _WorldSpaceCameraPos), 1);
            ...
            float3 eyeRay = normalize(mul((float3x3)unity_ObjectToWorld, vvertex.xyz));
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        half4 frag (xv2f v) : SV_Target
        {
            boxConditionalClipLocal(v.posLocal.xyz);
            v2f IN = vertpixel(v);
